// 검증 이력 데이터 — 다른 리포트에 쓸 땐 이 객체만 교체
export const PROVENANCE = {
  author: { name:'유민호', title:'실장', org:'마드라스체크', dept:'AI사업개발실' },
  period: { from:'2026-07-22(수)', to:'07-26(일)', mode:'실작업 3일(22·23·26)', effort:'6시간 39분 실측' },
  tool: 'Claude Opus 5',
  asOf: '2026-07-26',
  use: '내부 검토용',           // 용도 — 등급 엄격도의 기준. 서명줄에 남는다
  rounds: 6,                    // 교차검증 라운드 수. 칩의 "N× reviewed" 는 이 값이다
  method: '웹 검색 20+회 · HuggingFace config.json·공식 요금표 직접 조회',

  stats: [
    { v:'105', l:'프롬프트' },
    { v:'6차',  l:'교차검증' },
    { v:'6시간 39분', l:'실투입' },
    { v:'3일',  l:'실작업(44·15·46)' },
  ],

  // 트랜스크립트 타임스탬프(UTC)를 KST 로 변환해 30분 이상 공백을 경계로 끊은 작업 구간.
  // 자기 서술이 아니라 로그다. 날짜는 그룹 헤더로 빼고 행에는 시각만 둔다.
  timeline: [
    { d:'07-22', wd:'수', rows:[
      { t:'11:18', min:23,  n:4,  k:'GPU 단위 정리',     s:'"1대"가 칩 1장인지 서버 8장인지. 보드(SXM/PCIe) 차이까지' },
      { t:'13:41', min:59,  n:15, k:'가격·구매루트 확정', s:'AWS 온디맨드 vs Elice vs 단품 구매. 1장 단위 대여 가능 여부' },
      { t:'21:57', min:109, n:25, k:'계산기로 전환',      s:'"돌린다"의 정의 재정립 → 격자표를 계산기로. 기업 케이스 매칭은 근거 부족으로 폐기' },
    ]},
    { d:'07-23', wd:'목', rows:[
      { t:'09:15', min:0,   n:1,  k:'라벨 정리',         s:'MoE/Dense 표기가 혼란만 줘서 제거' },
      { t:'14:16', min:10,  n:3,  k:'로고·정렬 도입',     s:'모델 식별성 확보, 대여/구매 3종 동시 표시' },
      { t:'15:07', min:0,   n:1,  k:'3박스 레이아웃',     s:'모델·설정 위, 결과 아래로 구조 확정' },
      { t:'16:16', min:0,   n:1,  k:'렌더 확인',         s:'스크린샷 대조' },
      { t:'17:07', min:55,  n:8,  k:'리패턴 퀵필터',      s:'실제 요건(툴콜 65+·64K·INT8)을 프리셋으로. Qwen3.8·Granite 편입' },
      { t:'18:41', min:0,   n:1,  k:'Granite 반영',      s:'모델 표 갱신' },
    ]},
    { d:'07-26', wd:'일', rows:[
      { t:'11:14', min:114, n:33, k:'재설계 + 할루시네이션 4연타', s:'표 재구성·정밀도 근거 확인 후 "한번 더"만 4회 — TFLOPS·KV캐시 오류가 여기서 나옴' },
      { t:'14:22', min:16,  n:6,  k:'신뢰 등급 도입',     s:'툴콜링 강/중 3단계 → 점수화. "컨설팅에 써도 되냐"는 질문에서 A/B/C/D 체계가 나옴' },
      { t:'15:32', min:13,  n:7,  k:'FP8 기본화·경고 배치', s:'품질 손실 미미해 기본값으로. "반드시 실측하라"를 최상단에' },
    ]},
  ],

  // 타임라인이 "언제 무엇을 했나" 를 말하므로, 여기엔 겹치지 않는 **확정된 판단**만 남긴다.
  angles: [
    ['보드가 성능을 가른다', '같은 H100도 SXM(NVLink 900GB/s, 실효율 75~85%) vs PCIe(64GB/s, 20~30%).'],
    ['대여 vs 구매 손익분기', 'H100 기준 AWS 6개월·Elice 14.5개월. 상시 풀가동이냐 간헐이냐로 갈림.'],
    ['MoE 착시', '활성 3B라도 전체 30B가 VRAM에 상주. 활성 파라미터는 속도지 용량이 아님.'],
    ['MLA 구조 발견', 'Kanana2·DeepSeek V3.2는 압축 latent만 저장. KV캐시가 통념의 절반.'],
  ],

  // 항목별 신뢰 등급 (A 그대로 인용 / B 상대 비교만 / C 자릿수만 / D 미검증)
  grades: [
    { k:'VRAM·동시 슬롯', g:'A', why:'물리 계산 + config.json 1차 확인' },
    { k:'가격',          g:'B', why:'공개 요금표 기반, 환율·시세 가정 포함' },
    { k:'TTFT·RPM',      g:'B', why:'MFU·디코드 효율이 가정치' },
    { k:'품질점수',       g:'C', why:'27종 중 4종만 실측, 나머지는 순위 환산' },
  ],

  // 실제로 틀려서 고친 것 — now 로 현재 값과 연결해야 검증 가능해진다
  findings: [
    { r:'2차', sev:'치명', t:'TFLOPS가 FP16이 아닌 sparse/FP8 값(990→495)', now:'현재 H100=495 · GPU 표에서 확인' },
    { r:'3차', sev:'치명', t:'KV캐시 임의값 → 아키텍처 공식으로 재산출(최대 4.5배 오차)', now:'VRAM 상세에 산출식 표기' },
    { r:'5차', sev:'중대', t:'AWS H200/B200 가격 동일값 오류' },
  ],

  // 1차 출처 — "믿지 말고 찍어보라"
  sources: [
    { t:'Solar Open 100B config.json', u:'https://huggingface.co/upstage/Solar-Open-100B/raw/main/config.json', n:'48층·KV8·hd128 → KV 0.188MB', r:'공개 문서' },
    { t:'AWS p6-b200 요금',            u:'https://instances.vantage.sh/aws/ec2/p6-b200.48xlarge',              n:'$113.93/hr ÷8 = $14.24', r:'공개 문서' },
    { t:'BFCL v4 리더보드',            u:'https://gorilla.cs.berkeley.edu/leaderboard.html',                    n:'툴콜링 점수 스케일 앵커', r:'공개 문서' },
    { t:'ES 인덱스별 primary 바이트', u:null, n:'comment_hybrid 1,510 GiB', r:'GET .monitoring-es-*/_search {type:index_stats}' }
  ],

  // 아직 못 한 것 — 다음 담당자용
  gaps: [
    { item:'성능 실측', sub:'TTFT·RPM 절대값', why:'GPU 미보유 (효율 계수가 가정치)', gain:'H200 반나절 + vLLM 30분 → B → A 승격' },
    { item:'툴콜 성공률', sub:'품질점수 근거', why:'자체 평가셋 부재', gain:'후보 3개 × 자체 데이터 → C → B 승격' },
  ],

  closing: '위 항목은 도구로는 못 올린다. 실제 환경·데이터가 있어야 한다. 그 전까지 이 자료는 후보를 좁히는 용도로만 쓴다.',
};
